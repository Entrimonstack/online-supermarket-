...
if (!process.env.MONGODB_URL || !process.env.STRIPE_SECRET_KEY || !process.env.FRONTEND_URL) {
...
app.post("/login", async (req, res) => {
  try {
    ...
    if (user) {
      ...
      return res.status(404).json({ message: "Email not found, please sign up", alert: false });
    }
  } catch (err) {
    console.error("Login error:", err);
    res.status(500).json({ message: "Internal Server Error" });
